...
namespace TMtextng.KeyboardPages
{
    ...
    /// </summary>
    public partial class NumericKeyboard : Page
    {
        public static NumericKeyboard globalNumericKeyboard;
        ...
        public NumericKeyboard()
        {
            ...
            Button[,] LowerABC_Button = new Button[4, 12];


            buttonLetter = iniReader.numericKeyboardLetter;
            ...
            TextBlock[,] LowerABC_TextBlock = new TextBlock[4, 12];
            Viewbox[,] LowerABC_Viewbox = new Viewbox[4, 12];
            ...
            for (int i = 0; i < configureButton.Length; i++)
            {
                ...
                if (configureButton[i].Content.ToString() == "Vorl")
                {
                    if (iniReader.textReadMode == "3")
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 12; j++)
                {
                    ...
                    else if (valueFromIniReader == '/'.ToString()) { valueFromIniReader = "schrägstrich"; }
                    else if (valueFromIniReader == '='.ToString()) { valueFromIniReader = "gleichheitszeichen"; }
                    else if (valueFromIniReader == '('.ToString()) { valueFromIniReader = "klammerAuf"; }
                    else if (valueFromIniReader == ')'.ToString()) { valueFromIniReader = "klammerZu"; }
                    else if (valueFromIniReader == ":") { valueFromIniReader = "Doppelpunkt"; }
                    else if (valueFromIniReader == "§") { valueFromIniReader = "absatz"; }
                    else if (valueFromIniReader == "@") { valueFromIniReader = "et"; }
                    else if (valueFromIniReader == "$") { valueFromIniReader = "dollar"; }
                    else if (valueFromIniReader == "#") { valueFromIniReader = "raute"; }
                    else if (valueFromIniReader == "|") { valueFromIniReader = "senkrechtstrich"; }
                    else if (valueFromIniReader == @"\") { valueFromIniReader = "Backslash"; }
                    else if (valueFromIniReader == "~") { valueFromIniReader = "tilda"; }
                    ...
                    //MessageBox.Show(LowerABC_TextBlock[i, j].Text);
                    ...
                    if (iniReader.numericButtonImages[buttonCount] == 1)
                    {
                    ...
                    {
                        LowerABC_Button[i, j].CommandParameter = "S-ON NumericKeyboard";
                    }
                    else if (valueFromIniReader == "S-PAU")
                    {
                        LowerABC_Button[i, j].CommandParameter = "S-PAU NumericKeyboard";
                    }
                    else if (valueFromIniReader == "S-OFF")
                    {
                        LowerABC_Button[i, j].CommandParameter = "S-OFF NumericKeyboard";
                    }
                    else
        ...
        public async void TextChangedEventHandler(object sender, TextChangedEventArgs args)
        {
            ...
            for (int i = 0; i < lineCount; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    if (words[i, j].StartsWith(globalNumericKeyboard.TextBoxContent.Text.Split(' ').Last(), StringComparison.OrdinalIgnoreCase))
                    {
            ...
            for(int i = 0; i < globalNumericKeyboard.WordSuggestionGrid.Children.Count; i++)
            {
                if (globalNumericKeyboard.TextBoxContent.Text.Split(' ').Last() != "" && count < suggestedWordsList.Count)
                {
                    globalNumericKeyboard.WordSuggestionGrid.Children[i].Visibility = Visibility.Visible;
                ...
                {
                    globalNumericKeyboard.WordSuggestionGrid.Children[i].Visibility = Visibility.Hidden;
